...
for (int i = 0; i < n; i++) {
    for (int j = wt[i]; j <= W; j++) { // FORWARD loop
...
14. Decode ways
    ...
    int numDecodings(string s) {
     ...
     ```
        ...
        vector<int> dp(n + 1, 0);
        dp[0] = 1;  // Empty string
        dp[1] = 1;  // First char is guaranteed non-'0' here

        for (int i = 2; i <= n; i++) {
            // One-digit check (s[i-1])
            if (s[i - 1] != '0')
                dp[i] += dp[i - 1];

            // Two-digit check (s[i-2..i-1])
     ...
     ```
        ...
        for (auto& dir : directions) {
            ...
            if (x >= 0 && x < matrix.size() && y >= 0 && y < matrix[0].size() &&
                ...
                max_path =
                    max(max_path, 1 + dfs(matrix, memory, x, y, directions));
            }
        }
...
58. Unique Paths II
     ```
        ...
        if (dp[r][c] != -1)
            return dp[r][c];
        return dp[r][c] = rec(r, c + 1, obstacleGrid, m, n, dp) +
                          rec(r + 1, c, obstacleGrid, m, n, dp);
    }
...
63. Maximum Profit in Job Scheduling
```
...
65. Longest happy prefix
     ...
     ```
66. Count Square Submatrices with All Ones
      ```
